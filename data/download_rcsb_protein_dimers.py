...
def candidate_from_assembly(assembly: Dict[str, Any]) -> Optional[Candidate]:
    ...
    if len(entities) == 1:
        ...
        entity_records: List[Tuple[str, str]] = []
        for pe in entities:
            ...
            if not entity_id or cluster_id is None:
                print(f"Skipping {assembly_id}: missing protein-entity or cluster information", file=sys.stderr)
                return None
            entity_records.append((entity_id, str(cluster_id)))
...
def main() -> int:
    ...
    print("Fetching assembly/entity metadata and 100% sequence-cluster memberships...", file=sys.stderr)
