...
def main():
    ...
    int_df["is_duplicate"] = int_df.duplicated(subset=["tmp_cluster_pair"], keep=False)

    # Filter out duplicate interactions, keeping only one representative for each cluster pair
    filtered_int_df = int_df[~int_df["is_duplicate"]].copy()
    ...
    filtered_int_df.drop(columns=["tmp_cluster_pair", "is_duplicate"], inplace=True)
